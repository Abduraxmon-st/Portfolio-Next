...
export const ContactButton = ({ children = "Contact", className }: { children?: React.ReactNode, className?: string }) => {
  ...
  return (
    <Button
      ...
      className={`relative overflow-hidden py-2! flex! gap-3 pl-7! pr-5! h-12 text-lg rounded-3xl hover:scale-105 border border-[#242f6a50] hover:border-[#292c41] active:scale-95 transition-transform duration-300 transform group ${className}`}
    >
      ...
      <ArrowRight className='relative z-10 group-hover:translate-x-1 transition-all' size={18} />
    </Button>
  )
}
